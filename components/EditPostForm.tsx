...
export default function EditPostForm({ post }: { post: PostData }) {
  ...
  const [type, setType] = useState<'club' | 'study' | 'team'>(post.type as 'club' | 'study' | 'team')
  const [applyMode, setApplyMode] = useState<'form' | 'link'>(post.applyMode as 'form' | 'link')
  const [posterPreview, setPosterPreview] = useState<string | null>(post.poster)
  const [roles, setRoles] = useState<Role[]>(
    post.roles.length > 0
      ? post.roles.map((r) => ({ id: String(++roleIdCounter.current), name: r.name, count: r.count }))
      : [newRole()]
  )
  const [showApplicantCount, setShowApplicantCount] = useState(post.showApplicantCount)
  const [form, setForm] = useState({
    title: post.title,
    field: post.field,
    capacity: post.capacity,
    current: post.current,
    deadline: new Date(post.deadline).toISOString().split('T')[0],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/posts/${post.id}/manage`)
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
      <h1 className="text-xl font-bold text-gray-900 mb-6">모집글 수정</h1>
